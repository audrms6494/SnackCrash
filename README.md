...
- 함영주
    - 기능적으로 미숙해서 팀에 큰 도움이 되지 못한 것 같아 스스로 아쉽다.
    - Input System과 캐릭터 이동에 대해 구현,  타일 맵, Scene간 이동 시도를 해보면서 이전에 들었던 강의와 개인과제를 리마인드 할 수 있었고,  UI 부분을 정렬하는 과정도 재미있었다.
    - 작업을 하면서  많지는 않지만 진행 내용을 공유하지 않고 작업을 진행 하는 과정에  깃허브 컴플릭트가 일어났는데,  자기가 어떤 작업을 진행 할건지, 진행 중인지 팀원들과 공유하는 과정이 협업에서 중요한 부분임을 다시 한 번 느꼈다.
    - 마지막으로  팀원분들이 전부 긍정적이셔서 큰 문제 없이 프로젝트를 잘 마칠 수 있어서 좋았다 우리팀 최고:두_손으로_만든_하트:
