...
public class BlockSpawnManager : MonoBehaviour
{
    ...
    public void SpawnUti(int x, int y, int CIndex)
    {
        ...
        GameObject block = Instantiate(BlockPrefab[0], spawnPosition, Quaternion.identity);
        block.GetComponent<Block>().SetManager(this);
        ChangeBlockColor(block, CIndex);
        Blocks.Add(block);
    }
    public void SpawnUti(int x, int y, int CIndex, int BIndex)
    {
        BlockClear = false;
        Vector3 spawnPosition;
        spawnPosition = Spawnpoint_Block.position + new Vector3(x * 1.1f, y * -0.3f, 0f);
        GameObject block = Instantiate(BlockPrefab[BIndex], spawnPosition, Quaternion.identity);
        block.GetComponent<HardBlock>().SetManager(this);
        Blocks.Add(block);
    }
